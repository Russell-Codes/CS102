...
public class AIPlayer {
    ...
    public boolean takeTurn(Game game, Player player) {
        // Generated by Claude Haiku 4.5; main AI turn logic with prioritized actions
        // (buy > reserve > take coins).

        // Following methods have also been generated with the help of Claude Haiku 4.5
        // as part of the overall AI implementation
    ...
    private boolean tryBuyBestCard(Game game, Player player) {
        // Scan both visible cards and player's reserved hand for best affordable option
        Card bestCard = null;
    ...
    private boolean tryReserveCard(Game game, Player player) {
        // Reserve strategy: if reserve limit not hit, take a high-value card (value >=
        // 2)
        ...
        if (isSuccessful) {
            // If gold taken and AI over 10-coin limit, auto-discard excess
    ...
    private void autoDiscard(Game game, Player player) {
        // Find which color has highest excess (i.e., coins > needed for target card)
        ...
        // Discard one coin of the most-excess color
        if (bestIdx >= 0) {
    ...
    private boolean tryTakeCoins(Game game, Player player) {
        // Compute target card needs to guide color selection priority
    ...
    private int[] computeNeeds(Game game, Player player) {
        // Identify the cheapest visible card (minimum total gems needed after
        // bonuses/coins)
        ...
        for (Card currentCard : game.getVisibleCards()) {
            if (currentCard == null)
                continue;
            // Calculate total gems still needed for this card
            int total = 0;
            for (int i = 0; i < REGULAR_GEM_TYPES; i++) {
                total += Math.max(0, currentCard.getCost()[i] - player.getBonuses()[i] - player.getCoins()[i]);
            }
            // Track card with minimum need (closest to affordability)
        ...
        // Compute exact gem shortfall for the target card
        if (targetCard != null) {
            for (int i = 0; i < REGULAR_GEM_TYPES; i++) {
